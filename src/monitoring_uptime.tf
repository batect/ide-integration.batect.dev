...
resource "google_monitoring_uptime_check_config" "api_config_schema_endpoint" {
  display_name = "API (/v1/configSchema.json)"
  timeout      = "10s"
  period       = "600s"

  http_check {
    path         = "/v1/configSchema.json"
    use_ssl      = true
    validate_ssl = true
  }
...
resource "google_monitoring_alert_policy" "api_config_schema_endpoint" {
  display_name = "API /v1/configSchema.json policy"
  combiner     = "OR"
